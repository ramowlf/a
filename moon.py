...
def kimlik_sorgu(message):
    try:
        ...
        if query:
            url = f"https://sowixapi.online/api/sowixapi/adsoyadil.php?{'&'.join([f'{key}={value}' for key, value in query.items()])}"
            ...
            bot.reply_to(message, "Geçersiz komut. kullanım: /sorgu -ad Adnan -soyad Oktar -il Karabük ")
    except IndexError:
        bot.reply_to(message, "Geçersiz komut kullanım. /sorgu -ad Adnan -soyad Oktar -il Karabük ")
import requests
...
def unblock_user(message):
    ...
    try:
        ...
        bot.reply_to(message, 'anasini sikmek istediğini kişinin ID\'si gir. böyle kullan  /kaldir <kullanıcı_id>')
        return
...
def start(message):
    ...
    bot.reply_to(message, "👋 Merhaba botumuza hoşgeldin ilk defa başlattıyorsan 250000 TL bakiye başlangıç hediyesi olarak verilir İyi oyunlar. Komutlar /yardim", reply_markup=markup)
...
def send_balance(message):
    ...
    save_balances()
    bot.reply_to(message, f'Başarılı! {target_id} kimlikli kullanıcıya {amount} TL bakiye gönderildi. Yeni bakiye: {user_balances[target_id]} TLnız.')
...
def send_balance(message):
    ...
    user_balances[target_id] -= amount
    save_balances()
    bot.reply_to(message, f'Başarılı! {target_id} kimlikli kullanicidan {amount} TL bakiye alindi. Yeni bakiye: {user_balances[target_id]} TLnız.')
...
def free(message):
    ...
    try:
        ...
        for key, value in balances.items():
            if value == 0:
                user_balances[key] = 500000

        save_balances()
        bot.reply_to(message, "Tüm uygun kullanıcılara 500000 bakiye gönderildi.")
