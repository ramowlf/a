...
def trigger_upload(client, message):
    user_id = message.from_user.id
    if user_id not in [member.user.id for member in bot.get_chat_members(channel_username)]:
        bot.send_message(
            chat_id=message.chat.id,
            text="Botu başlatmak için önce kanala katılmanız gerekiyor."
        )
        return
